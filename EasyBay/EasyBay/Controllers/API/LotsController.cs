...
namespace EasyBay.Controllers.API
{
    ...
    public class LotsController : ControllerBase
    {
        ...
        [HttpPost("all")]
        public IEnumerable<Lot> All(
            [FromForm]int page = 0, [FromForm]int pagesize = 10)
        {
            return facade.GetActualLots().Skip(
        ...
        {
            facade.CreateNewLot(User.Identity.Name,
                request.Name, request.Description,
                request.StartingPrice, request.BuyOutPrice,
                request.TradeFinishTime, request.Tags);
        }
        ...
        public void Edit([FromBody]EditLotRequest request)
        {
            if (User.IsInRole(Role.Admin) 
                || facade.GetOwnedLots(
                    User.Identity.Name).Any(l => l.Id == request.Id))
                facade.EditLot(request.Id, request.Name,
                    request.Description, request.BuyOutPrice,
                    request.TradeFinishTime, request.Tags);
        }
